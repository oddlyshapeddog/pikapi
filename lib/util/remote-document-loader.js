...
const DEFAULT_REMOTE_FILE_FORMAT = process.env.DEFAULT_REMOTE_FILE_FORMAT || 'cson'
...
function detectFormat(url) {
  ...
  ext = ext.split('.').pop()
  return ext in FORMATTERS ? ext : DEFAULT_REMOTE_FILE_FORMAT
}
...
function handleRemoteResponse(response, formatter) {
  return new Promise((resolve, reject) => {
    ...
    response.on('end', () => {
      winston.debug(`Response: ${body.substring(0, 140)}...`)
      formatter(body)
        .then(
          resolve,
          reject
...
function loadAndParse(url, format) {
  format = format || detectFormat(url)
  return new Promise((resolve, reject) => {
