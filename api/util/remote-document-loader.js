...
// constants
const MAX_REMOTE_FILE_SIZE = process.env.MAX_REMOTE_FILE_SIZE || 64 * 1024 // 32 kb
...
function handleRemoteResponse(response, formatter) {
  return new Promise((resolve, reject) => {
    ...
    response.on('end', () => {
      winston.debug(`Response: ${body.substring(0, 512)}...`)
      formatter(body)
        .then(
          resolve,
          reject
...
function loadAndParse(url, options) {
  ...
  const format = options.format || detectFormat(url) || options.defaultFormat
  return new Promise((resolve, reject) => {
